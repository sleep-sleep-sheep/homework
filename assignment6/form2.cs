    ...
    public partial class form2 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
           ...
           if( orderDetails.Count!=0)
            {
                if(MyClass.orderService.orders.Count==0)
                {
                ...
                else if (MyClass.orderService.orders.Count != 0)
                {
                    Order order = new Order(MyClass.orderService.orders[MyClass.orderService.orders.Count-1].Id+1, customer, DateTime.Now);
                    foreach(OrderDetail detail in orderDetails)
                    {
                        order.AddDetails(detail);
                    }
